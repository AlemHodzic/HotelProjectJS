 ...
 //scroll animacije, provjera da li je user scrollo dole dovoljno da bi header fade-out ili fade-in ukoliko se user vrati
 function checkScroll(){
    let pocetak = $('.navbar').height() * 3; // varijablu pocetak postavljamo kao visinu navbara * 3

    if($(window).scrollTop() > pocetak){ //provjera da li je user scrollo dole dovoljno
        $('.navbar').addClass("scrolled"); //dodajemo predefinisanu klasu ''scrolled'' 
    ...
    if($('.navbar').length > 0){
      ...
      let i = 0;
      function changer(){ //postavlja background slike svake 3 sekunde
         if(i==0){
      ...
      output.innerHTML = slider.value;
         slider.oninput = function() { //kombinacija jquery i DOM-a, appendam sobe tako sto se slajder mijenja onInput. sobe uopste nisu u HTMLu vec se appendaju i brišu ukoliko ne zadovoljavaju kriterije slajdera i ako nisu slobodne
      ...
      function loadRooms(){
         for(let i=0;i<sobe.length;i++){ // prvobitna funkcija koja se poziva na loadu stranice
      ...
      let kvalue = "-";
      brKreveta.onchange = function(){ //provjera broja kreveta i appendanje soba kada se promijeni broj kreveta
         setTimeout(start,200);
         slider.min = maxc;
         $(".ind-room").remove();
      ...
      function closecontent(){
         $(".rez-container").css("display", "none"); //mijenjanje displaya na kliku X-a
